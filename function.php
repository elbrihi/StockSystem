...
function fill_customer_list_byId($connect,$id)
{
	$query = "
...
function count_sum_caiss($connect)
{
	$date = date('Y-m-d');
	
	$query = "
	SELECT sum(montant)  as total_caisse_value FROM  caisse
	WHERE date = '$date' 
	AND caisse_status='active'
	";
	...
	foreach($result as $row)
	{
		return number_format($row['total_caisse_value'], 2);
		
	}
}
function count_sum_caiss_sortie($connect)
{
	$date = date('Y-m-d');
	...
	SELECT sum(montant)  as total_caisse_value FROM  caisse
	WHERE date = '$date' 
	AND caisse_status='active' AND type = 'sortie'
	";
	...
	foreach($result as $row)
	{
		return $row['total_caisse_value'];
		
	}
}
function count_sum_caiss_entree($connect)
{
	$date = date('Y-m-d');
	...
	SELECT sum(montant)  as total_caisse_value FROM  caisse
	WHERE date = '$date' 
	AND caisse_status='active' AND type = 'entree'
	";
...
function  count_profit_store($connect)
{
	$query = "SELECT SUM(profit) FROM inventory_order_product ";
