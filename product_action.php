...
if(isset($_POST['btn_action']))
{
	...
	{
		$query = "
		INSERT INTO product (category_id, brand_id, product_name, product_description, product_quantity, prix_achat, product_unit, product_base_price, product_tax, product_enter_by, product_status, product_date) 
		VALUES (:category_id, :brand_id, :product_name, :product_description, :product_quantity,:prix_achat, :product_unit, :product_base_price, :product_tax, :product_enter_by, :product_status, :product_date)
		";
		...
		{ 
			...
			$statement->execute(
				array(
					':do'	=> $_SESSION['user_name'].' a entree  ' .$_POST['product_quantity']. ' ' .	$_POST['product_unit']. ' du produit ' . $_POST["product_name"].' à la date ' . date("Y-m-d"),
					':date' => date("Y-m-d"),
				)
			);
		}
	...
	if($_POST['btn_action'] == 'Edit')
	{
		...
		if(isset($result))
		{
			echo 'Product Details Edited';
				...
				$statement->execute(
					array(
						':do'	=> $_SESSION['user_name'].' a modifiée   le produit ' . $_POST["product_name"].' à la date ' . date("Y-m-d"),
						':date' => date("Y-m-d"),
					)
				);
		}
